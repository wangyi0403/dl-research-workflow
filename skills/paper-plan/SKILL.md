...
- **REVIEWER_MODEL = `claude-sonnet-4-6`** — Model used via Agent self-review for outline review.
...
2. **docs/12_auto_review.md** — auto-review loop conclusions
3. **Experiment results** — JSON files in `figures/`, screen logs, tables
4. **docs/03_idea_report.md** — from idea-discovery pipeline (if applicable)
5. **Compact files** (if available): `docs/idea_candidates.md`, `docs/08_findings.md`, `docs/07_result_record.md` — preferred over full files when present, saves context window
...
**First check for `docs/09_claims.md`** — if it exists (generated by `/result-to-claim` at the end of Workflow 2), use it as the starting point for claims. This file contains validated claims already mapped to experiment evidence. Merge with any additional claims from the narrative documents below.

If `docs/09_claims.md` does not exist, extract claims from scratch:
...
Send the complete outline to Agent for feedback:
...
Save the final outline to `docs/13_paper_plan.md` in the project root:
