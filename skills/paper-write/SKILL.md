...
- **REVIEWER_MODEL = `claude-sonnet-4-6`** — Model used via Agent self-review for section review.
- **TARGET_VENUE = `ICLR`** — Default venue. Supported: `ICLR`, `NeurIPS`, `ICML`, `CVPR` (also ICCV/ECCV), `ACL` (also EMNLP/NAACL), `AAAI`, `ACM` (ACM MM, SIGIR, KDD, CHI, etc.), `IEEE_JOURNAL` (IEEE Transactions / Letters, e.g., T-PAMI, JSAC, TWC, TCOM, TSP, TIP), `IEEE_CONF` (IEEE conferences, e.g., ICC, GLOBECOM, INFOCOM, ICASSP), `ELSEVIER_DC` (Elsevier双栏，如 Transportation Research Part C / Expert Systems with Applications), `ELSEVIER_SC` (Elsevier单栏). Determines style file and formatting.
...
1. **docs/13_paper_plan.md** — outline with claims-evidence matrix, section plan, figure plan (from `/paper-plan`)
...
If no docs/13_paper_plan.md exists, ask the user to run `/paper-plan` first or provide a brief outline.
...
**ELSEVIER_DC** (双栏，Transportation Research Part C / Expert Systems with Applications 等):
```latex
\documentclass[a4paper,fleqn]{cas-dc}
\usepackage[authoryear,longnamesfirst]{natbib}
% 引用格式：\citep{} / \citet{}，不用 \cite{}
% 需要：cas-dc.cls, cas-common.sty, cas-model2-names.bst
```
- 使用 `templates/elsevier_dc.tex` 作为 main.tex 框架
- 作者信息用 `\author[1]{Name}[type=author]` 格式
- 关键词用 `\sep` 分隔
- 参考文献：`\bibliographystyle{cas-model2-names}`
- **ANONYMOUS = false**（Elsevier 不匿名投稿）

**ELSEVIER_SC** (单栏):
```latex
\documentclass[a4paper,fleqn]{cas-sc}
\usepackage[authoryear,longnamesfirst]{natbib}
% 需要：cas-sc.cls, cas-common.sty, cas-model2-names.bst
```
- 使用 `templates/elsevier_sc.tex` 作为 main.tex 框架
...
├── iclr2026_conference.sty     # or neurips_2025.sty / icml2025.sty / IEEEtran.cls+bst / cas-dc.cls+cas-sc.cls+cas-common.sty+cas-model2-names.bst
...
2. Copy venue template from `templates/`:
   - `ICLR/NeurIPS/ICML/CVPR/ACL/AAAI/ACM` → copy corresponding `.sty` file + use venue `.tex` as main.tex
   - `IEEE_JOURNAL / IEEE_CONF` → copy `IEEEtran.cls` + `IEEEtran.bst`
   - `ELSEVIER_DC` → copy `cas-dc.cls`, `cas-sc.cls`, `cas-common.sty`, `cas-model2-names.bst`；main.tex 用 `templates/elsevier_dc.tex`；**ANONYMOUS = false**
   - `ELSEVIER_SC` → 同上，main.tex 用 `templates/elsevier_sc.tex`；**ANONYMOUS = false**
...
Before drafting the front matter, re-read the one-sentence contribution from `docs/13_paper_plan.md`. The Abstract and Introduction should make that takeaway obvious before the reader reaches the full method.
...
Send the complete draft to the Agent for review:
