...
User input (PROBLEM + vague APPROACH)
  ...
  -> Phase 2 (Agent self-review, Opus critic): Review for fidelity, specificity, contribution quality, and frontier leverage
  -> Phase 3 (Claude): Anchor check + simplicity check -> revise method -> rewrite full proposal
  -> Phase 4 (Agent, include Phase 2 review as context): Re-evaluate revised proposal
...
- **REVIEWER_MODEL = `claude-sonnet-4-6`** — Reviewer model used via Agent self-review.
...
| `review_context` | string or null | Reviewer thread ID for `codex-reply` continuity |
...
2. **On resume**, read the state file and recover context:
   ...
   - Recover `review_context` for reviewer thread continuity
...
**Checkpoint:** Write `refine-logs/REFINE_STATE.json` with `{"phase": "anchor", "round": 0, "review_context": null, "last_score": null, "last_verdict": null, "status": "in_progress", "timestamp": "<now>"}`.
...
Send the full proposal to the Agent for an **elegance-first, frontier-aware, method-first** review. The reviewer should spend most of the critique budget on the method itself, not on expanding the experiment menu.
...
**CRITICAL: Save the `review_context`** from this call for all later rounds.
...
**Checkpoint:** Update `refine-logs/REFINE_STATE.json` with `{"phase": "review", "round": 1, "review_context": "<saved>", "last_score": <parsed>, "last_verdict": "<parsed>", ...}`.
...
Send the revised proposal back to the Agent in the **same thread**:
...
Agent prompt:
  You are a rigorous external reviewer. Be adversarial and honest.
  You are continuing a multi-round review. Include your previous assessment from the review document as context.
...
**Checkpoint:** Update `refine-logs/REFINE_STATE.json` with `{"phase": "review", "round": N, "review_context": "<saved>", "last_score": <parsed>, "last_verdict": "<parsed>", ...}`.
...
#### Step 5.1: Write `docs/review_summary.md`
...
#### Step 5.2: Write `docs/04_final_proposal.md`
...
#### Step 5.3: Write `docs/refinement_report.md`
...
- Final proposal: `docs/04_final_proposal.md`
...
- Canonical clean version lives in `docs/04_final_proposal.md`
...
[Full verbatim response from Agent]
...
Review summary: docs/review_summary.md
Full report: docs/refinement_report.md
Final proposal: docs/04_final_proposal.md
...
- **ALWAYS use `config: {"model_reasoning_effort": "xhigh"}`** for all Agent review calls.
- **Save `review_context` from Phase 2** and use a new Agent call with previous review as context
