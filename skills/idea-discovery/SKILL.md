...
Each phase builds on the previous one's output. The final deliverables are a validated `docs/03_idea_report.md` with ranked ideas, plus a refined proposal (`docs/04_final_proposal.md`) and experiment plan (`docs/05_experiment_plan.md`) for the top idea.
...
- **REVIEWER_MODEL = `claude-sonnet-4-6`** — Model used via Agent self-review. Passed to sub-skills.
- **ARXIV_DOWNLOAD = false** — When `true`, `/research-lit` downloads the top relevant arXiv PDFs during Phase 1. When `false` (default), only fetches metadata. Passed through to `/research-lit`.
- **COMPACT = false** — When `true`, generate compact summary files for short-context models and session recovery. Writes `docs/idea_candidates.md` (top 3-5 ideas only) at the end of this workflow. Downstream skills read this instead of the full `docs/03_idea_report.md`.
- **REF_PAPER = false** — Reference paper to base ideas on. Accepts: local PDF path, arXiv URL, or any paper URL. When set, the paper is summarized first (`docs/ref_paper_summary.md`), then idea generation uses it as context. Combine with `base repo` for "improve this paper with this codebase" workflows.
...
4. **Generate `docs/ref_paper_summary.md`**:
...
Phase 1 and Phase 2 will use `docs/ref_paper_summary.md` as additional context — `/research-lit` searches for related and competing work, `/idea-creator` generates ideas that build on or improve the reference paper.
...
Invoke `/idea-creator` with the landscape context (and `docs/ref_paper_summary.md` if available):
...
**What this does:**
- If `docs/ref_paper_summary.md` exists, include it as context — ideas should build on, improve, or extend the reference paper
- Brainstorm 8-12 concrete ideas via Claude critic sub-agent
...
- Output `docs/03_idea_report.md`

**🚦 Checkpoint:** Present `docs/03_idea_report.md` ranked ideas to the user. Ask:
...
- Cross-verify with Claude critic sub-agent
- Check for concurrent work (last 3-6 months)
- Identify closest existing work and differentiation points

**Update `docs/03_idea_report.md`** with deep novelty results. Eliminate any idea that turns out to be already published.
...
**What this does:**
- Agent acts as a senior reviewer (NeurIPS/ICML level)
- Scores the idea, identifies weaknesses, suggests minimum viable improvements
- Provides concrete feedback on experimental design

**Update `docs/03_idea_report.md`** with reviewer feedback and revised plan.
...
```
/research-refine + /experiment-plan "[top idea description + pilot results + reviewer feedback]"
```

**What this does:**
- Freeze a **Problem Anchor** to prevent scope drift
- Iteratively refine the method via Agent review (up to 5 rounds, until score ≥ 9)
- Generate a claim-driven experiment roadmap with ablations, budgets, and run order
- Output: `docs/04_final_proposal.md`, `docs/05_experiment_plan.md`, `docs/06_experiment_tracker.md`
...
Finalize `docs/03_idea_report.md` with all accumulated information:
...
## Refined Proposal
- Proposal: `docs/04_final_proposal.md`
- Experiment plan: `docs/05_experiment_plan.md`
- Tracker: `docs/06_experiment_tracker.md`
...
Write `docs/idea_candidates.md` — a lean summary of the top 3-5 surviving ideas:
...
This file is intentionally small (~30 lines) so downstream skills and session recovery can read it without loading the full `docs/03_idea_report.md` (~200+ lines).
