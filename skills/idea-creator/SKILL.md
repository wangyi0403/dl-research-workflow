...
- **REVIEWER_MODEL = `claude-sonnet-4-6`** — Model used via Agent self-review for brainstorming and review.
...
Use the external LLM via Agent self-review for divergent thinking:
...
Save the review_context for follow-up.
...
1. **Novelty check**: Use the `/novelty-check` workflow (multi-source search + Agent cross-verification) for each idea

2. **Critical review**: Use Agent (with previous review context) (same thread):
   ```
...
3. **Combine rankings**: Merge your assessment with Agent's ranking. Select top 2-3 ideas for pilot experiments.
...
Write a structured report to `docs/03_idea_report.md` in the project root:
