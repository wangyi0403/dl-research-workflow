...
docs/05_experiment_plan.md   →   implement → Agent review → deploy → collect → initial results ready
docs/06_experiment_tracker.md     code        (cross-model)  /run-experiment     for /auto-review-loop
docs/04_final_proposal.md
```

## Constants

- **CODE_REVIEW = true** — Agent reviews experiment code before deployment. Catches logic bugs before wasting GPU hours. Set `false` to skip.
...
- **COMPACT = false** — When `true`, (1) read `docs/idea_candidates.md` instead of full `docs/03_idea_report.md` if available, (2) append experiment results to `docs/07_result_record.md` after collection.
...
1. **`docs/05_experiment_plan.md`** (best) — claim-driven experiment roadmap from `/experiment-plan`
2. **`docs/06_experiment_tracker.md`** — run-by-run execution table
3. **`docs/04_final_proposal.md`** — method description for implementation context
4. **`docs/idea_candidates.md`** — compact idea summary (preferred when `COMPACT: true`)
5. **`docs/03_idea_report.md`** — full brainstorm output (fallback)
...
Before deploying, send the experiment code to Agent for review:
...
- **Agent self-review unavailable** → skip silently, proceed to Phase 3 (graceful degradation)
...
3. **Update `docs/06_experiment_tracker.md`** — fill in Status and Notes columns
...
**Date**: [today]
**Plan**: docs/05_experiment_plan.md
...
Append each completed experiment to `docs/07_result_record.md`:
...
- Append ablation blocks to `docs/05_experiment_plan.md` and `docs/06_experiment_tracker.md`
...
Results: docs/07_result_record.md
...
- **Update the tracker.** `docs/06_experiment_tracker.md` should reflect real status after each run completes.
