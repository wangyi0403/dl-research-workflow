...
description: Design ablation studies from a reviewer's perspective. Preferred use is upfront (after method is confirmed, before experiments run) to avoid post-hoc rationalization. Can also be used after result-to-claim to add supplementary ablations. Agent sub-agent leads design, CC reviews feasibility.
...
Systematically design ablation studies that answer the questions reviewers will ask. Agent sub-agent leads the design (reviewer perspective), CC reviews feasibility and implements.

## Context: $ARGUMENTS

## When to Use

- **Preferred — upfront design**: After method is confirmed (idea-discovery complete, docs/04_final_proposal.md exists) but before experiments run. Reads the method and experiment plan to design ablations proactively.
- After main results pass `/result-to-claim` to add supplementary ablations
- When `/auto-review-loop` reviewer identifies missing ablations

When used upfront, skip reading result files (none exist yet) and base ablation design purely on method components from docs/04_final_proposal.md and docs/05_experiment_plan.md.
...
- Method description and components (from docs/05_experiment_plan.md or project CLAUDE.md)
- Current experiment results (from docs/07_result_record.md, docs/06_experiment_tracker.md, or W&B)
...
### Step 2: Agent Designs Ablations

Use the **Agent tool** to spawn a critic sub-agent:

```
Agent prompt:
  You are a rigorous external reviewer. Be adversarial and honest.
...
Normalize Agent response into structured format:
...
- Cuts: if budget is tight, propose removing lower-priority ablations and ask Agent to confirm
...
4. Track results in docs/07_result_record.md
5. After all ablations complete → update docs/08_findings.md with insights

## Rules

- **The Agent sub-agent leads the design. CC does not pre-filter or bias the ablation list** before the Agent sees it. The Agent thinks like a reviewer; CC thinks like an engineer.
...
- If total compute exceeds budget, CC proposes cuts and asks Agent to re-prioritize — don't silently drop ablations.
...
- Record all ablation results in docs/07_result_record.md, including negative results (component removal had no effect = important finding).
